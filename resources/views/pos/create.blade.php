...
<head>
    ...
    <script src="{{ asset('assets/kasir/js/script.js') }}"></script>
</head>
    ...
    <div class="hide-print flex flex-row h-screen antialiased text-blue-gray-800">
                    ...
                    <div class="h-full overflow-y-auto px-2">
                        ...
                        <div x-show="filteredProducts().length" class="grid grid-cols-4 gap-4 pb-3">
                            <template x-for="product in filteredProducts()" :key="product.id">
                                <div role="button"
                                    class="select-none cursor-pointer transition-shadow overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-lg transform hover:scale-105"
                                    :title="product.name" x-on:click="addToCart(product)">
                                    <img :src="product.image" :alt="product.name"
                                        class="rounded-t-2xl h-48 w-full object-cover">
                                    <div class="flex flex-col pb-4 px-4 text-sm">
                                        <p class="flex-grow truncate font-medium text-gray-800" x-text="product.name">
                                        </p>
                                        <p class="mt-1 font-semibold text-gray-600" x-text="priceFormat(product.price)">
                                        </p>
                                    </div>
                                </div>
                            </template>
        ...
        <div x-show="isShowModalReceipt"
            ...
            <div x-show="isShowModalReceipt" class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10"
                ...
                <form method="POST" action="{{ route('pos.store') }}" x-ref="checkoutForm">
                    @csrf
                    <!-- form content -->
                    ...
                    <div class="p-4 w-full">
                        <button type="button"
                            class="bg-blue-500 text-white text-lg px-4 py-3 rounded-2xl w-full focus:outline-none"
